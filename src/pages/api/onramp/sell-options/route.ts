import {
  type FetchSellOptions,
  type OnrampSellOptionsSnakeCaseResponse,
...
type OnrampSellOptionsResponseRaw = OnrampSellOptionsSnakeCaseResponse;
type OnrampSellOptionsResponse = Awaited<ReturnType<FetchSellOptions>>;

/**
 * Fetches available sell options (payment currencies and sellable assets) for offramp
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Call CDP API to get sell options
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: errorData.message || "Failed to fetch sell options" },
    ...
    const data: OnrampSellOptionsResponseRaw = await response.json();
    const dataCamelCase: OnrampSellOptionsResponse =
